    ...
    public class CheckoutTests : PageTest
    {
         public static double ParsePrice(string text)
        {
            if (string.IsNullOrWhiteSpace(text))
                throw new FormatException("Price element contained no text.");
        ...
        public async Task TestCheckoutProcessWithDiscount()
        {
            ...
            var priceElement = Page.Locator(".cart-total");
             await priceElement.WaitForAsync(new LocatorWaitForOptions
            {
                State   = WaitForSelectorState.Visible,
                Timeout = 10000
            });
            ...
            var originalPrice = ParsePrice(priceText);
           ...
           await Page.Locator(".discount-applied").WaitForAsync(new LocatorWaitForOptions
            ...
            var finalPrice = ParsePrice(finalPriceText);
